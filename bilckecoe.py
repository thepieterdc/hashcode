...
def eindscore(result, ritten, bonus):
    ...
    for carindex, carrides in enumerate(result):
        ...
        for ride in carrides:
            if tijd + distance(vorigeplaats, ritten[ride].start) > ritten[ride].laststart:
                print("oesje")
...
def score(ride: Rit, car: Car, bonus):
    ...
    half = grid_size // 2
    ret = 0
    carslecht = distance(car.endlocation, ride.start)
    ret -= carslecht
...
def insert(cars, car):
    ...
    while first <= last and not found:
        ...
        if cars[midpoint].readytime == find:
            found = True
        else:
            if cars[midpoint].readytime > find:
                last = midpoint - 1
...
def algoritme(ritten, number_of_vehicles, bonus, outp):
    ...
    while len(ritten) > 0:
        ...
        optimalecarindex = -1
        optimalescore = -1000000000
        optimalecar = -1000000000
        ...
        if optimalecarindex != -1:
            ...
            starttijd = max(ride.earlyeststart,
                            cars[optimalecarindex].readytime + distance(cars[optimalecarindex].endlocation, ride.start))
            cars[optimalecarindex].readytime = starttijd + distance(ride.start, ride.end)
            cars[optimalecarindex].endlocation = ride.end
            cars = sorted(cars, key=lambda c: c.readytime)
        del ritten[0]
...
def runAll():
    ...
    for file in ["d_metropolis.in", "e_high_bonus.in"]:
        # for file in ["c_no_hurry.in"]:
        try:
            np.os.remove("{}.out".format(file))
        except:
            pass
